...
app.post('/api/settlements', authenticateToken, requireGodmode, async (req, res) => {
  try {
    ...
    const settlementData = {
      ...
      verified_at: new Date().toLocaleString('en-US', { timeZone: 'Asia/Makassar' }),
...
app.get('/api/settlements/carry-over', authenticateToken, requireGodmode, async (req, res) => {
  try {
...
app.post('/api/settings', authenticateToken, requireGodmode, async (req, res) => {
  try {
    const { google_sheets_webhook_url } = req.body;
    if (google_sheets_webhook_url !== undefined) {
      await db.updateSetting('google_sheets_webhook_url', google_sheets_webhook_url.trim());
    }
