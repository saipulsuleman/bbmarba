...
// Helper format timestamp untuk Supabase PostgreSQL (TIMESTAMPTZ)
function formatToTimestamptz(val) {
  if (!val) return new Date().toISOString();
  if (val instanceof Date) return val.toISOString();
  let str = String(val).trim();
  if (str.endsWith(' WITA')) {
    str = str.replace(' WITA', '').trim();
...
function cleanTxForSupabase(txData) {
  const allowed = [
    'transaction_no', 'plate_no', 'equipment_code', 'vehicle_type',
    'fuel_name', 'price_per_liter', 'total_rp', 'liters', 'receipt_no',
    'receipt_photo_url', 'driver_name', 'notes', 'filling_time_wita',
    'payment_status', 'is_void', 'void_reason', 'voided_by', 'voided_at',
    'idempotency_key', 'is_registered', 'ownership_group', 'settlement_id',
    'created_by', 'created_at', 'updated_at'
  ];
  const cleaned = {};
  for (const key of allowed) {
    if (txData[key] !== undefined) {
      cleaned[key] = txData[key];
    }
  }
  if (txData.id && UUID_REGEX.test(txData.id)) {
    cleaned.id = txData.id;
  }
  if (cleaned.filling_time_wita) {
    cleaned.filling_time_wita = formatToTimestamptz(cleaned.filling_time_wita);
  }
  if (cleaned.created_at) {
    cleaned.created_at = formatToTimestamptz(cleaned.created_at);
  }
  if (cleaned.updated_at) {
    cleaned.updated_at = formatToTimestamptz(cleaned.updated_at);
  }
  if (cleaned.voided_at) {
    cleaned.voided_at = formatToTimestamptz(cleaned.voided_at);
  }
  return cleaned;
}

function cleanSettlementForSupabase(sData) {
  const allowed = [
    'settlement_no', 'settlement_date', 'total_transactions_amount',
    'transfer_amount', 'variance_amount', 'variance_status', 'variance_notes',
    'bank_name', 'bank_ref_no', 'transfer_proof_url', 'verified_by',
    'verified_at', 'notes'
  ];
  const cleaned = {};
  for (const key of allowed) {
    if (sData[key] !== undefined) {
      cleaned[key] = sData[key];
    }
  }
  if (sData.id && UUID_REGEX.test(sData.id)) {
    cleaned.id = sData.id;
  }
  if (cleaned.verified_at) {
    cleaned.verified_at = formatToTimestamptz(cleaned.verified_at);
  }
  return cleaned;
}
...
const db = {
  ...
  async getTransactions(filter = {}) {
    if (supabase) {
      try {
        ...
        if (filter.date) {
          const startOfDay = `${filter.date}T00:00:00+08:00`;
          const endOfDay = `${filter.date}T23:59:59+08:00`;
          query = query.gte('filling_time_wita', startOfDay).lte('filling_time_wita', endOfDay);
        }
        ...
        if (!error && data && data.length > 0) return data;
        if (!error && data && data.length === 0 && !filter.status && !filter.plate_no && !filter.date) {
          const local = readLocalDb();
          if (local.transactions && local.transactions.length > 0) {
            return local.transactions.sort((a, b) => new Date(b.filling_time_wita || b.created_at) - new Date(a.filling_time_wita || a.created_at));
          }
        }
        if (!error && data) return data;
      } catch (err) {
        console.warn('[Database] Exception query transactions Supabase:', err.message);
      }
    }
    ...
    result.sort((a, b) => new Date(b.filling_time_wita || b.created_at) - new Date(a.filling_time_wita || a.created_at));
    return result;
  },
  ...
  async createTransaction(txData) {
    ...
    if (supabase) {
      try {
        const payload = cleanTxForSupabase(txData);
        const { data, error } = await supabase.from('transactions').insert([payload]).select().single();
        if (!error && data) {
          // Sync with local memory/storage
          local.transactions.unshift({ ...newTx, id: data.id });
          writeLocalDb(local);
          return data;
        }
        console.warn('[Database] Gagal insert transaksi Supabase, fallback local:', error?.message);
      } catch (err) {
        console.warn('[Database] Exception insert transaksi Supabase:', err.message);
      }
    }
  ...
  async createSettlement(settlementData, transactionIds) {
    if (supabase) {
      try {
        const payload = cleanSettlementForSupabase(settlementData);
        // 1. Insert settlement
        const { data: sData, error: sErr } = await supabase.from('settlements').insert([payload]).select().single();
        if (!sErr && sData) {
        ...
        console.warn('[Database] Exception insert settlement Supabase:', err.message);
      }
    }
